...
class PointCloudExample
{
  ...
  private:
    ...
    void	extract_color_from_cloud(const cloud_cp& cloud)		;
...
PointCloudExample::PointCloudExample(ros::NodeHandle& nh)
    :_cloud_sub(nh.subscribe("/pointcloud", 1,
     ...
     _color(new sensor_msgs::Image)
{
}

void
PointCloudExample::run()
{
    ros::spin();
}
...
main(int argc, char* argv[])
{
    ...
    {
	ros::NodeHandle					nh("~");
	depth_camera_tutorial::PointCloudExample	pointcloud_example(nh);

	pointcloud_example.run();
    }
